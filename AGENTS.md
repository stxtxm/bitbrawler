...
- **Issues créées par un agent ne trigger jamais `issues:opened/edited`** (events GITHUB_TOKEN bloqués par GitHub) → sweeper `oc-sweeper.yml` (*/10 min) dispatch les `/oc` sans label `oc-dipatched` ; dédup par PR `feat/auto-N`

...
**PLAN DE CONTINGENCE (si la feature est trop grande)**:
Si l'issue est trop complexe pour être implémentée en une seule fois:
...
3. Utilise: `gh issue create --title "feat: [sub-task N/M] Description" --body "Description... /oc" --label auto-generated`
4. **STOP** après avoir créé les sous-issues — n'implémente RIEN toi-même
5. Le workflow GitHub dispatchera automatiquement chaque sous-issue au dev-agent
6. **NE PAS** commencer à implémenter — chaque sous-issue sera traitée indépendamment
7. **NE PAS DEMANDER** "Souhaitez-vous que je procède" — FAIS-LE, c'est tout
...
- ✅ TOUJOURS agir et implémenter
- ✅ Si tu proposes un plan, EXÉCUTE-LE immédiatement

**OBLIGATION FORMELLE**:
- ✅ Tu DOIS coder la solution (ou la découper en sous-issues avec /oc)

...
function getComebackCount(history: Character['fightHistory']): number {
  ...
  // Iterate from oldest to newest: we need to detect wins AFTER 2+ consecutive losses
...
export const applyMedalReward = (character: Character, reward: MedalReward): Character => {
  ...
  switch (reward.type) {
    ...
    case 'stat_point': {
      const bonus = reward.value ?? 1;
      // Handle "all stats" reward first (no stat field — e.g. peak_performance)
      if (reward.label?.includes('all stats')) {
        ...
        updated[reward.stat] = (updated[reward.stat] || 10) + bonus;
      }
      break;
    }
    case 'inventory_slot': {

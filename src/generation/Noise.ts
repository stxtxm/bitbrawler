export function noise2D(x: number, y: number, seed: number): number {
  ...
  for(let i=0; i<256; i++) p[i] = p[i+256] = permutation[(i + seed) % 256];
...
export function generateNoiseMap(width: number, height: number, seed: number): Uint8Array {
  const map = new Uint8Array(width * height);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const val = noise2D(x * 0.05, y * 0.05, seed);

...
export const SceneBox = memo(function SceneBox({
  ...
  // The biome is driven by the character's raid progress: once the first boss
  // is slain the PvE training ground trades its scrolling plains for the
  // volcanic arena — the world visibly warms up.
  const biome = getBiomeForCharacter(character);

  return (
    <div className="scene-box">
      {biome.id === 'volcanic' ? (
        <BiomeTerrain seed={character.seed} animated={pveMode} />
      ) : (
        <ProceduralTerrain
          seed={character.seed}
          animated={pveMode}
